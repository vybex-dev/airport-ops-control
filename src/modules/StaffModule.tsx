...
import { useDataset } from "@/lib/sim/dataLoader";
...
export const StaffModule: React.FC = () => {
  ...
  const { ready: staffReady } = useDataset("staff_shifts");

  const allShifts = useMemo(() => getAllStaffShifts(), [staffReady]);

  const kpis = useMemo(() => {
    return getStaffKPIs(currentTimeMs);
  }, [currentTimeMs, staffReady]);
    ...
    <div className="space-y-md">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-md p-md rounded-md bg-surface-1 border border-line">
        <div className="flex items-center gap-sm">
          ...
          <div>
            ...
            <p className="font-display text-xs text-ink-muted mt-4xs">
              Ground Handling Shifts, Real-Time Duty Derived Telemetry & Gate
              Deployment Roster
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-xs text-xs font-data text-ink-muted bg-surface-2 px-sm py-xs rounded border border-line">
          ...
          SIM TIME:{" "}
          <span className="text-accent-signal font-bold">{formattedTime}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-sm">
        <div className="p-sm rounded-md bg-surface-1 border border-line">
          ...
          <div className="text-[10px] font-data text-ink-muted mt-1">
            Ground Personnel
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-sm p-sm rounded-md bg-surface-1 border border-line">
        ...
        <div className="text-xs font-data text-ink-muted">
          Showing{" "}
          <strong className="text-ink-primary">{filteredShifts.length}</strong>{" "}
          of 600 records
        </div>
      </div>
          ...
          <table className="w-full min-w-[900px] text-left border-collapse font-data text-xs">
            ...
            <tbody className="divide-y divide-line/40">
              {filteredShifts.slice(0, 150).map((staff) => (
                ...
                >
                  <td className="p-sm font-bold text-ink-primary">
                    {staff.staffId}
                  </td>
                  <td className="p-sm text-ink-primary font-medium">
                    {staff.staffName}
                  </td>
                  <td className="p-sm">
                  ...
                  <td className="p-sm text-ink-muted">
                    {staff.shiftStartTimestamp.slice(11, 16)} -{" "}
                    {staff.shiftEndTimestamp.slice(11, 16)} ({staff.shiftHours}
                    h)
                  </td>
                  <td className="p-sm">
                    <StatusBadge
                      variant={staff.badgeVariant}
                      size="sm"
                      pulseDot={staff.isOnDuty}
                    >
                      {staff.shiftStatusLabel}
                    </StatusBadge>
                  </td>
                  <td className="p-sm text-ink-muted font-mono">
                    {staff.supervisorRef}
                  </td>
                  <td className="p-sm">
                    <span
                      className={`flex items-center gap-1 ${staff.isCertExpiringSoon ? "text-status-alert font-bold" : "text-ink-muted"}`}
                    >
                      {staff.isCertExpiringSoon && (
                        <Award className="h-3 w-3 text-status-alert" />
                      )}
