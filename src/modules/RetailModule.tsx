...
export const RetailModule: React.FC = () => {
  ...
  const { ready: retailReady } = useDataset("retail_transactions");

  const allTx = useMemo(() => getAllRetailTransactions(), [retailReady]);
  const kpis = useMemo(() => getRetailKPIs(), [retailReady]);
  const hourlyData = useMemo(() => getHourlyRetailTrend(), [retailReady]);
    ...
    <div className="space-y-md">
        ...
        <div className="flex items-center gap-sm">
          ...
          <div>
            ...
            <p className="font-display text-xs text-ink-muted mt-4xs">
              Duty-Free Concourse Sales Density, Passenger Spend Trends & Flight
              Spend Correlations
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-sm p-sm rounded-md bg-surface-1 border border-line">
        ...
        <div className="text-xs font-data text-ink-muted">
          Showing{" "}
          <strong className="text-ink-primary">{filteredTx.length}</strong> of
          3,000 transactions
        </div>
      </div>
