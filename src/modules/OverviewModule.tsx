...
export const OverviewModule: React.FC = () => {
  ...
  const { formattedTime, isPlaying, togglePlay, speedMultiplier } = useSimClock();
  const { alerts, unacknowledgedCount, openAlertsDrawer, acknowledgeAlert } = useAlerts();
  ...
  const getModuleCards = () => [
    {
      ...
      status: kpis.flights.delayed > 0 ? ('delayed' as const) : ('boarding' as const),
      ...
      description: 'Real-time FIDS schedule, departure delays, boarding events, and live turnaround states.',
    ...
    },
    {
      ...
      status: kpis.gates.activeConflicts > 0 ? ('alert' as const) : ('ontime' as const),
      statusLabel: kpis.gates.activeConflicts > 0 ? `${kpis.gates.activeConflicts} Gate Conflict` : 'All Gates Normal',
      ...
      description: 'Concourse timeline Gantt, turnaround tracking, buffer conflicts, and pushback status.',
    ...
    },
    {
      ...
      status: kpis.baggage.misroutedCount > 0 ? ('delayed' as const) : ('ontime' as const),
      ...
      description: 'Baggage lifecycle tracking, check-in to carousel delivery SLA, and discrepancy logs.',
    ...
    },
    {
      ...
      status: kpis.security.backlogRisk === 'HIGH' ? ('delayed' as const) : ('ontime' as const),
      ...
      description: 'Screening throughput, lane opening logs, wait-time telemetry, and clear audit scans.',
      accentBorder: 'ontime' as const,
    },
    {
      id: 'staff',
      ...
      description: 'Department shift schedules, terminal coverage, ground handling roster, and availability.',
      accentBorder: 'signal' as const,
    },
    {
      id: 'retail',
      ...
      description: 'Passenger retail transactions, duty-free sales telemetry, and gate-area footfall density.',
    ...
    },
    {
      ...
      status: kpis.maintenance.criticalDefects > 0 ? ('alert' as const) : ('neutral' as const),
      ...
      description: 'Technical maintenance logs, defect work orders, aircraft readiness, and technician entries.',
  ...
  const handleInvestigateAlert = (alert: SimAlert) => {
    if (alert.affectedFlightId) {
      navigate(`/flights?flightId=${encodeURIComponent(alert.affectedFlightId)}`);
      return;
    }
    switch (alert.source) {
      case 'flight':
  ...
  return (
    <div className="space-y-md font-display">
      ...
      <div className="p-md rounded-lg bg-surface-1 border border-line flex flex-col lg:flex-row lg:items-center justify-between gap-md shadow-lg relative overflow-hidden">
        ...
        <div className="space-y-xs relative z-10">
          <div className="flex items-center gap-xs flex-wrap">
            ...
            <StatusBadge
              variant={kpis.overviewStatus.level === 'CRITICAL' ? 'alert' : kpis.overviewStatus.level === 'WARNING' ? 'delayed' : 'ontime'}
              size="sm"
              pulseDot
            >
          ...
          <div>
            ...
            <p className="font-display text-xs text-ink-muted mt-0.5">
              {kpis.overviewStatus.description} Calculated dynamically over 11,827 pre-indexed operational records.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-sm flex-wrap relative z-10 shrink-0">
          <div className="p-2 rounded bg-surface-0 border border-line flex items-center gap-xs font-data text-xs">
            ...
            <div>
              <div className="text-ink-primary font-semibold tracking-tight">{formattedTime}</div>
            ...
            <button
              ...
              aria-label={isPlaying ? 'Pause simulation clock' : 'Play simulation clock'}
            >
              {isPlaying ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5 text-status-ontime" />}
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-md">
        ...
        <Panel variant="surface-1" accentBorder="boarding" className="p-md relative">
          ...
          <div className="font-data mb-xs">
            <div className="text-2xl font-bold text-ink-primary tracking-tight">
              ...
              <span className="text-xs font-normal text-ink-muted ml-1.5">AIRBORNE / ON-TARMAC</span>
            </div>
          </div>
          ...
          <div className="space-y-1 font-data text-xs pt-xs border-t border-line/50">
            ...
            <div className="flex items-center justify-between text-ink-muted">
              ...
              <span className={kpis.flights.delayed > 0 ? 'text-status-delayed font-semibold' : 'text-ink-primary'}>
        ...
        <Panel variant="surface-1" accentBorder="signal" className="p-md relative">
          <div className="flex items-center justify-between gap-xs mb-xs">
            <span className="font-data text-[10px] font-bold text-ink-muted uppercase tracking-wider">
              CONCOURSE & GATES
            </span>
          ...
          <div className="font-data mb-xs">
            <div className="text-2xl font-bold text-ink-primary tracking-tight">
              ...
              <span className="text-xs font-normal text-ink-muted ml-1.5">UTILIZATION</span>
            </div>
          </div>
          ...
          <div className="space-y-1 font-data text-xs pt-xs border-t border-line/50">
            <div className="flex items-center justify-between text-ink-muted">
              ...
              <span className="text-ink-primary font-semibold">
                <AnimatedNumber value={kpis.gates.occupiedGates} suffix=" / 50" />
              </span>
            </div>
            <div className="flex items-center justify-between text-ink-muted">
              <span>Gate Turnaround Conflicts</span>
              <span className={kpis.gates.activeConflicts > 0 ? 'text-status-alert font-bold animate-pulse' : 'text-status-ontime font-medium'}>
        ...
        <Panel variant="surface-1" accentBorder="ontime" className="p-md relative">
          <div className="flex items-center justify-between gap-xs mb-xs">
            <span className="font-data text-[10px] font-bold text-ink-muted uppercase tracking-wider">
          ...
          <div className="font-data mb-xs">
            <div className="text-2xl font-bold text-ink-primary tracking-tight">
              ...
              <span className="text-xs font-normal text-ink-muted ml-1.5">DELIVERY SLA</span>
            </div>
          </div>
          ...
          <div className="space-y-1 font-data text-xs pt-xs border-t border-line/50">
            ...
            <div className="flex items-center justify-between text-ink-muted">
              ...
              <span className={kpis.baggage.misroutedCount > 0 ? 'text-status-delayed font-semibold' : 'text-status-ontime'}>
        ...
        <Panel variant="surface-1" accentBorder="ontime" className="p-md relative">
          ...
          <div className="font-data mb-xs">
            <div className="text-2xl font-bold text-ink-primary tracking-tight">
              ...
              <span className="text-xs font-normal text-ink-muted ml-1.5">AVG QUEUE WAIT</span>
            </div>
          </div>
          ...
          <div className="space-y-1 font-data text-xs pt-xs border-t border-line/50">
            <div className="flex items-center justify-between text-ink-muted">
              ...
              <span className="text-ink-primary font-semibold">
                <AnimatedNumber value={kpis.security.activeLanes} suffix={` / ${kpis.security.totalLanes}`} />
              </span>
            </div>
            <div className="flex items-center justify-between text-ink-muted">
          ...
          <div className="flex-1 overflow-y-auto p-sm space-y-xs">
            {topCurrentAlerts.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-md text-center text-ink-muted my-auto h-full">
                ...
                <span className="font-data text-xs font-semibold text-ink-primary">Zero Active Critical Alerts</span>
                ...
                >
                  <div className="flex items-start justify-between gap-xs mb-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="font-data text-[10px] text-accent-signal">{alert.timestamp}</span>
                    </div>

                    <span className="font-data text-[10px] text-ink-muted">{alert.ruleId}</span>
                  </div>

                  <h4 className="font-display text-xs font-bold text-ink-primary mb-1">{alert.title}</h4>
                  <p className="font-display text-[11px] text-ink-muted line-clamp-2 mb-2">{alert.description}</p>
                  ...
                  <div className="flex items-center justify-between pt-xs border-t border-line/40">
                    <div className="font-data text-[10px] text-ink-primary font-medium">
                      {alert.affectedFlightId ? `Flight ${alert.affectedFlightId}` : alert.affectedRef || alert.source}
                    </div>
      ...
      <div className="pt-2xs flex items-center justify-between">
        <div>
          ...
          <p className="font-display text-xs text-ink-muted">
            Access specialized sub-system dashboards for deep-dive operational management.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-md">
        {getModuleCards().map((mod) => (
          ...
          >
            <div className="flex flex-col h-full justify-between gap-md" onClick={() => navigate(mod.path)}>
              {/* Card Top */}
              <div>
                ...
                <p className="font-display text-xs text-ink-muted line-clamp-2">{mod.description}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-xs p-xs rounded bg-surface-2/60 border border-line/50 font-data">
                <div>
                  <div className="text-[10px] text-ink-muted uppercase">{mod.primaryMetricLabel}</div>
                  <div className="text-sm font-semibold text-ink-primary">{mod.primaryMetric}</div>
                </div>
                <div>
                  <div className="text-[10px] text-ink-muted uppercase">{mod.secondaryMetricLabel}</div>
                  <div className="text-sm font-semibold text-ink-primary">{mod.secondaryMetric}</div>
                </div>
              </div>

              {/* Card Bottom */}
              <div className="flex items-center justify-between pt-xs border-t border-line/50">
                <span className="font-data text-xs text-ink-muted">{mod.recordCount}</span>
                <Button
                  variant="ghost"
                  size="xs"
                  icon={<ArrowRight className="h-3.5 w-3.5 group-hover:translate-x-1 transition-transform" />}
                  iconPosition="right"
                >
                  Open Shell
                </Button>
